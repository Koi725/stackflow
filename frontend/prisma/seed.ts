...
async function main() {
  ...
  const cards: {
    ...
    label: Label; priority: Priority; column: Column; owner: string;
  }[] = [
    ...
    { id: "seed-106", title: "Drag drops cards on touch", description: "Cards jump back on iOS when the finger leaves the column early.", label: "bug", priority: "med", column: "todo", owner: "riley@stackflow.dev" },
  ...
  for (const c of cards) {
    ...
    const data = { title: c.title, description: c.description, label: c.label, priority: c.priority, column: c.column, ownerId };
